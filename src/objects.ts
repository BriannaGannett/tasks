...
export function toShortForm(question: Question): string {
    ...
    let shortName: String = question3.name.slice(0, 10);
    return question3.id + ": " + shortName;
}
...
export function toMarkdown(question: Question): string {
    ...
    let formattedQuestion: string =
        "# " + question4.name + "\n" + question4.body;
    if (question4.type === "multiple_choice_question") {
        const optionsFormatted = question4.options
            .map((option) => `\n- ${option}`)
            .join("");
        formattedQuestion += optionsFormatted;
    }
...
export function renameQuestion(question: Question, newName: string): Question {
    const question5 = { ...question, name: newName };
    return question5;
}
...
export function publishQuestion(question: Question): Question {
    const question6 = { ...question, published: !question.published };
    return question6;
}
